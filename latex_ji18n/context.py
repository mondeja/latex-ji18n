...
class ProjectContext(BaseContext):
    ...
    def source_dirpath(self):
        return os.path.join(self.project_path,
                            self._get_meta("source_dirname"))
    
    @property
    def dist_dirpath(self):
        return os.path.join(self.project_path,
                            self._get_meta("dist_dirname"))
    ...
    def _prepare(self):
        ...
        # config/
        config_dirpath = os.path.join(project_path,
                                      self._get_meta("config_dirname"))
        self._prepare_file(config_dirpath, "config_dirpath")
        
        # config/_private/
        config_private_dirpath = os.path.join(config_dirpath,
                                              self._get_meta("private_dirname"))
        self._prepare_file(config_private_dirpath, "config_private_dirpath")
        
        # config/data.yml
        data_filename = self._get_meta("data_filename")
        self._prepare_file(os.path.join(config_dirpath, data_filename),
                           "data_filepath")
        
        # config/_private/data.yml
        self._prepare_file(os.path.join(config_private_dirpath, data_filename),
                           "data_private_filepath")
        
        # config/layout.yml
        layout_filename = self._get_meta("layout_filename")
        self._prepare_file(os.path.join(config_dirpath, layout_filename),
                           "layout_filepath")
        ...
        self._prepare_file(os.path.join(config_dirpath, style_filename),
                           "style_filepath")
        
        # config/_private/style.yml
        self._prepare_file(os.path.join(config_private_dirpath, style_filename),
                           "style_private_filepath")
            
        # src/
        source_dirpath = os.path.join(project_path,
                                      self._get_meta("source_dirname"))
        self._prepare_file(source_dirpath, "source_dirpath", must_exists=True, 
                           not_exists_msg=('Source directory \'%s\' does not'
                                           ' exists.') % source_dirpath)
            
        # src/main.template.tex
        template_filepath = os.path.join(source_dirpath,
                                         self._get_meta("template_filename"))
        self._prepare_file(template_filepath,
                           "template_filepath", must_exists=True, 
                           not_exists_msg=('Template file \'%s\' does not'
                                           ' exists.') % template_filepath)
        
        # dist/
        dist_dirpath = os.path.join(project_path,
                                    self._get_meta("dist_dirname"))
        self._prepare_file(dist_dirpath, "dist_dirpath", must_exists=True, 
                           not_exists_msg=('Distribution directory \'%s\' does'
                                           ' not exists.') % dist_dirpath)
        
        # _i18n/
        i18n_dirpath = os.path.join(project_path,
                                    self._get_meta("i18n_dirname"))
        self._prepare_file(i18n_dirpath, "i18n_dirpath", must_exists=True, 
                           not_exists_msg=('i18n directory \'%s\' does'
                                           ' not exists.') % i18n_dirpath)
        self._prepared = True
    ...
    def load(self):
        ...
        for data_file_meta_prop in data_file_meta_props:
            ...
            if data_file_meta_prop in self[self._metadata_dict]:
                partial_context = read_yaml_file(
                    self._get_meta(data_file_meta_prop))
...
class LanguageContext(BaseContext):
    ...
    def _prepare(self):
        ...
        self._prepare_file(i18n_filepath, "i18n_filepath", must_exists=True, 
                           not_exists_msg=('i18n file \'%s\' does not'
                                           ' exists.') % i18n_filepath)    
    ...
    def _check_forbidden_attrs(self, data):
        ...
        for forbidden_attr in forbidden_attrs:
            if forbidden_attr in data:
                msg = ('The file \'%s\' can not contain an attribute'
                       ' \'%s\', you must use the file \'%s\' instead.') % (
                    self._get_meta("i18n_filepath"), forbidden_attr,
