...
def render(filepath=None,
    ...
    renderer = LaTeXJi18nRenderer(environment=environment,
                                  filters=filters)
    ...
    for language in languages:
        # Load context
        language_context = LanguageContext(language=language,
                                           project_context=project_context)
        language_context.load()
        
        # Render template to localized output
        localized_tex_filepath = os.path.join(
            project_context.source_dirpath,
            language_context.localized_template_name)
        localized_tex_filename = os.path.basename(localized_tex_filepath)
        renderer.render(template_filepath,
                        context=language_context,
                        destpath=localized_tex_filepath)
        ...
        expected_filepath = os.path.join(project_context.source_dirpath,
                                         expected_filename)
        if not os.path.exists(expected_filepath):
            break
        
        dist_filepath = os.path.join(project_context.dist_dirpath,
                                     expected_filename)
